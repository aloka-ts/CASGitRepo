...
import AgentModule._LogManagementInfStub;
...
public class SasDDHandler extends DefaultDDHandler {
	
	private static Logger logger = Logger.getLogger(SasDDHandler.class);
	...
	public void parse(DeploymentDescriptor dd, DeployableObject deployable) 
		...
		boolean nsaUpgrade = Boolean.parseBoolean(m_configRepository.getValue(Constants.PROP_NSA_UPGRADE));
        ...
        if(deployable.getDeploymentName() == null) {
        	digester.addBeanPropertySetter("sas-app/name", "deploymentName");
        }
        // Bug 6815
        if(deployable.getObjectName() == null) { // <app-name> from sip.xml must have been already read
        	digester.addBeanPropertySetter("sas-app/name", "objectName");
        }
        if(nsaUpgrade) {
        	digester.addBeanPropertySetter("sas-app/version", "fullVersion");
        } else {
        	digester.addBeanPropertySetter("sas-app/version", "version");
        	digester.addBeanPropertySetter("sas-app/version", "fullVersion");
        }
        digester.addBeanPropertySetter("sas-app/priority", "priority");
        digester.addBeanPropertySetter("sas-app/xmlapp", "xmlApp");
	    digester.addBeanPropertySetter("sas-app/main-servlet", "mainServlet");
        ...
        //JSR 289.42
        digester.addRule("sas-app/enable-annotation", annotationRule);
        digester.addRule("sas-app/enable-lib-annotation", annotationRule);
        ...
        digester.addRule("sas-app/measurement-config-file",appMeasurementRule);
        ...
        digester.addRule("sas-app/listener/listener-class", listenerRule);
        ...
        digester.addRule("sas-app/message-handler/handler-name", msgHandlerRule);
        digester.addRule("sas-app/message-handler/handler-class", msgHandlerRule);

        //Add the Resource Factory mapping as attributes in the Servlet Context.
		/*
		digester.addCallMethod("sas-app/resource-factory-mapping", "setAttribute", 2);
        digester.addObjectCreate("sas-app/resource-factory-mapping", ResourceFactoryHolder.class.getName());
        digester.addBeanPropertySetter("sas-app/resource-factory-mapping/resource-name", "resourceName");
        digester.addCallParam("sas-app/resource-factory-mapping/factory-name", 0);
        digester.addCallParam("sas-app/resource-factory-mapping", 1, true);
		*/

		if(deployable instanceof AseContext){
			ResourceFactoryMappingRule rfRule = new ResourceFactoryMappingRule(
												(AseContext)deployable);
			digester.addRule("sas-app/resource-factory-mapping/factory-name",rfRule);
			digester.addRule("sas-app/resource-factory-mapping/resource-name",rfRule);
		}

        //Make the Message Handler mappings
        digester.addCallMethod("sas-app/message-handler-mapping", "addDefaultHandler", 2);
        digester.addCallParam("sas-app/message-handler-mapping/resource-name", 0);
        digester.addCallParam("sas-app/message-handler-mapping/handler-name", 1);
        digester.addObjectCreate("sas-app/message-handler-mapping", ResourceMappingRule.class.getName());
        digester.addBeanPropertySetter("sas-app/message-handler-mapping/resource-name", "resourceName");
        digester.addBeanPropertySetter("sas-app/message-handler-mapping/handler-name", "servletName");
        digester.addSetNext("sas-app/message-handler-mapping", "addTriggeringRule", "com.baypackets.ase.dispatcher.Rule");
        ...
        digester.addRule("sas-app/login-config/auth-method", securityRule);
        digester.addRule("sas-app/login-config/realm-name", securityRule);
        ...
        digester.addRule("sas-app/replication-config/replication-policy", replConfigRule);
        digester.addBeanPropertySetter("sas-app/replication-config/replication-policy/event", "replicationEventId");
        digester.addRule("sas-app/replication-config/replication-policy/type", replConfigRule);
        
        //Process the response-time tag 
        digester.addObjectCreate("sas-app/response-time", TimeMeasurementRule.class.getName());
        digester.addSetProperty("sas-app/response-time", "target", "targetTime");
        digester.addSetProperty("sas-app/response-time", "weight", "weight");
        digester.addSetProperty("sas-app/response-time/start-time", "session-index", "beginSessionIndex");
        digester.addSetProperty("sas-app/response-time/start-time", "message-index", "beginMessageIndex");
        digester.addSetProperty("sas-app/response-time/end-time", "session-index", "endSessionIndex");
        digester.addSetProperty("sas-app/response-time/end-time", "message-index", "endMessageIndex");
        digester.addSetNext("sas-app/response-time", "addTimeMeasurementRule", TimeMeasurementRule.class.getName());

        digester.parse(new InputSource(dd.getStream()));
         		
        if(nsaUpgrade) {
        	String fullVersion = ((AbstractDeployableObject)deployable).getFullVersion();
        	if(fullVersion!=null && fullVersion.contains(AseStrings.PERIOD)) {
     			String[] verSplit = fullVersion.split("\\.");
     			fullVersion = verSplit[0]+AseStrings.PERIOD;
     			if(verSplit.length>1){
     				fullVersion= fullVersion+verSplit[1];
     			}
        	} 
        	((AbstractDeployableObject)deployable).setVersion(fullVersion);
        }
        
        if(logger.isDebugEnabled()){
    		logger.debug("getXmlApp : " + ((AbstractDeployableObject)deployable).isXmlApp());
    	}
        ...
        public void body(String nameSpace, String name, String body) throws SAXException {            
            try {
                ...
                if (name.equals("handler-name")) {
                    ...
                    if (logger.isDebugEnabled()) {
						logger.debug("call sethandler on wrapper class :"+ handler);
					}
					...
					//servlet = (Servlet)servletClazz.newInstance();                        
					//}
					if (handler instanceof Servlet && !servletClass.getName().equals("com.genband.jain.protocol.ss7.tcap.JainTcapProviderImpl")) {

						if (logger.isDebugEnabled()) {
							logger.debug("This handler is instance of servlet so set this as servlet on wrapper "+ servletClass.getName());
						}

...
public class XsltTransform extends DefaultHandler
{
	...
	private void transform (String classname) throws Exception	{
		...
		try	{
			...
			if(logger.isEnabledFor(Level.INFO))
			{
				logger.debug("First Line read is "+line);
			}
			
             if (line.indexOf("<?xml") != -1 && line.indexOf("request") != -1) {
				
				int xmlDecClose=line.indexOf(">");
				line=line.substring((xmlDecClose+1),line.length());
				
				if(logger.isEnabledFor(Level.INFO))
				{
					logger.debug("write First Line read after xml dec"+line);
				}
				fout.write (line);
				...
				if(logger.isEnabledFor(Level.INFO))
				{
					logger.debug("ignore xml declaration"+line);
				}
			}
			while ((line = fin.readLine()) != null) {
				if(logger.isEnabledFor(Level.INFO))
...
//				if(logger.isEnabledFor(Level.INFO))
//				{
//					logger.debug("Write Manually");
//				}
//				
//				fout.write("request.orig.host,request.orig.realm,");
//			}

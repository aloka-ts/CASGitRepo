...
import com.baypackets.ase.ra.diameter.gy.enums.AddresseeTypeEnum;
import com.traffix.openblox.core.exceptions.ValidationException;
import com.traffix.openblox.diameter.gy.generated.avp.AvpRecipientAddress;

public class RecipientAddressAvp extends AvpDiameterGrouped {

	private static Logger logger = Logger.getLogger(RecipientAddressAvp.class.getName());
	public static final long vendorId = 0L;

	private AvpRecipientAddress stackObj;

	public RecipientAddressAvp(AvpRecipientAddress stkObj){
		super(stkObj);
	...
	/**
	 *  Adding AddresseeType AVP of type Enumerated to the message.
	 */
	public AddresseeTypeAvp addAddresseeType(AddresseeTypeEnum value) throws GyResourceException {
		try {
			if(logger.isDebugEnabled()){
				logger.debug("Inside addAddresseeType()");
			}
			return new AddresseeTypeAvp(stackObj.addAddresseeType(AddresseeTypeEnum.getStackObj(value)));
		} catch (ValidationException e) {
			throw new GyResourceException("Exception in addAddresseeType",e);
		}
	}
	...
	/**
	 *  Retrieving a single Enumerated value from AddresseeType AVPs.
	 */
	public int getAddresseeType( ) throws GyResourceException {
		try {
			if(logger.isDebugEnabled()){
				logger.debug("Inside getAddresseeType()");
			}
			return stackObj.getAddresseeType();
		} catch (ValidationException e) {
			throw new GyResourceException("Exception in getAddresseeType",e);
		}
	}
	...
	/**
	 *  This method returns the enum value corrosponding to AddresseeTypeAvp
	 */
	public AddresseeTypeEnum getEnumAddresseeType( ) throws GyResourceException {
		try {
			if(logger.isDebugEnabled()){
				logger.debug("Inside getEnumAddresseeType()");
			}
			return AddresseeTypeEnum.getContainerObj(stackObj.getEnumAddresseeType());
		} catch (ValidationException e) {
			throw new GyResourceException("Exception in getEnumAddresseeType",e);
		}
	}

	/**
	 *  This method returns the enum value corrosponding to AddressTypeAvp
	 */

...
import com.baypackets.ase.ra.diameter.gy.avp.*;
...
import com.traffix.openblox.diameter.gy.generated.avp.*;

public class GyMessage extends AbstractSasMessage implements Message, Constants {

	private static Logger logger = Logger.getLogger(GyMessage.class);
	...
	public GyMessage(int type) {
		System.out.println("Inside ShMessage constructor ");
		this.type = type;
	}
	...
	public SasProtocolSession getProtocolSession(boolean create) {
		...
		if (create && GyResourceAdaptorImpl.getResourceContext() != null) {
			try {
				//this.session = this.context.getSessionFactory().createSession();
				this.session = GyResourceAdaptorImpl.getResourceContext().getSessionFactory().createSession();
			} catch (Exception e) {
	...
	public void send() throws IOException {
		...
		if(context != null){
			...
			try {
				GyResourceAdaptorFactory.getResourceAdaptor().sendMessage(this);
			} catch (Exception e) {
				logger.error("send(): " + e);

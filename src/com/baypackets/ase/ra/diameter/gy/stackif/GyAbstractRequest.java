...
import com.baypackets.ase.ra.diameter.gy.impl.GyResourceAdaptorImpl;
...
public abstract class GyAbstractRequest extends GyMessage implements GyRequest {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	private static Logger logger = Logger.getLogger(GyAbstractRequest.class);
	private static int count = 0;
	private int id;
	private boolean isReplicated = false ;
	// TODO need to add support for RoSession and StackSession.
	com.traffix.openblox.diameter.coding.DiameterRequest stackObj;
	private GySession m_session;

	public GyAbstractRequest() {
		super();
	}

	public GyAbstractRequest(int type) {
		super(type);
		System.out.println("Inside GyAbstractRequest(int) constructor ");
	}

	public GyAbstractRequest(GySession session){
		super();
		logger.debug("Inside GyAbstractRequest(GySession) constructor ");
	...
	public Response createResponse() throws GyResourceException {
		logger.debug("createResponse() is called.");
		try {
			//if (this.getResourceContext() != null ) {
			if(GyResourceAdaptorImpl.getResourceContext() != null) {
				//return (RoResponse)this.getResourceContext().getMessageFactory().createResponse(this, this.getType());
				return (GyResponse)GyResourceAdaptorImpl.getResourceContext().getMessageFactory().createResponse(this, this.getType());
			} else {
				logger.debug("Use default MessageFactory.");
				return (GyResponse)GyResourceAdaptorFactory.getMessageFactory().createResponse(this, this.getType());
			}
		} catch (Exception e) {
	...
	void setStackObject(com.traffix.openblox.diameter.coding.DiameterRequest stkObj){
		this.stackObj=stkObj;
	}

	com.traffix.openblox.diameter.coding.DiameterRequest getStackObject(){
	...
	//	//		throw new GyResourceException("Operation is not allowed");
	//	//	}
	//
	//
	//	public Response createResponse(int type) throws ResourceException {
	...
	//		if(GyResourceAdaptorImpl.getResourceContext() != null) {		
	//			//return (Response)this.getResourceContext().getMessageFactory().createResponse(this, type);
	//			return (Response)GyResourceAdaptorImpl.getResourceContext().getMessageFactory().createResponse(this, type);
	//		} else {
	//			logger.debug("Use default MessageFactory.");
	//			return (Response)GyResourceAdaptorFactory.getMessageFactory().createResponse(this, type);
	//		}
	//	}
	//
	//	public GyResponse createResponse() throws GyResourceException {
	//		logger.debug("createResponse() is called.");
	//		try {
	//			//if (this.getResourceContext() != null ) {
	//			if(GyResourceAdaptorImpl.getResourceContext() != null) {
	//				//return (GyResponse)this.getResourceContext().getMessageFactory().createResponse(this, this.getType());
	//				return (GyResponse)GyResourceAdaptorImpl.getResourceContext().getMessageFactory().createResponse(this, this.getType());
	//			} else {
	//				logger.debug("Use default MessageFactory.");
	//				return (GyResponse)GyResourceAdaptorFactory.getMessageFactory().createResponse(this, this.getType());
	...
	public java.lang.String getDestinationHost(){
		...
		try {
			...
			logger.error("Exception in getDestinationHost");
		}
		return destHost;
	}
	...
	public java.lang.String getDestinationRealm() throws ResourceException {
		try {
			...
			throw new ResourceException("Exception in getDestinationRealm ",e);
		}
	}


	public void setProxiable(boolean value) throws ResourceException {
		stackObj.setProxiable(value);
	}
...
//			containerObj = new  AuthorizedQoSAvp((AvpAuthorizedQoS) stackAvp);
...
//			containerObj = new  PDNConnectionIDAvp((AvpPDNConnectionID) stackAvp);

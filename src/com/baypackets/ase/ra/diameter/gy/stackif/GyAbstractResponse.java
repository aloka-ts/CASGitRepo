...
public abstract class GyAbstractResponse extends GyMessage implements GyResponse {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	private static Logger logger = Logger.getLogger(GyAbstractResponse.class);
	private static int count = 0;
	private int id;
	private boolean isReplicated = false ;
	// TODO need to add support for GySession and StackSession.
	DiameterAnswer stackObj;
	private GySession m_session;

	public GyAbstractResponse(int type) {
		super(type);
		System.out.println("Inside RoAbstractRequest constructor ");
	}

	public GyAbstractResponse(GySession session){
		super();
		this.m_session=session;
	}

	public GyAbstractResponse(DiameterAnswer stkObj){
		super();
	...
	public void send() throws IOException {
		logger.debug("GyAbstractResponse send() called.");

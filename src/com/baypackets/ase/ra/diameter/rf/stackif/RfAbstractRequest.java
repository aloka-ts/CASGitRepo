...
import com.baypackets.ase.ra.diameter.rf.impl.RfResourceAdaptorImpl;
...
public abstract class RfAbstractRequest extends RfMessage implements RfRequest {
	...
	public RfAbstractRequest() {
		super();
	}

	public RfAbstractRequest(int type) {
		super(type);
		System.out.println("Inside ShAbstractRequest(int) constructor ");
	}

	public RfAbstractRequest(RfSession session){
		super();
	...
	public Response createResponse() throws RfResourceException {
		logger.debug("createResponse() is called.");
		try {
			//if (this.getResourceContext() != null ) {
			if(RfResourceAdaptorImpl.getResourceContext() != null) {
				//return (ShResponse)this.getResourceContext().getMessageFactory().createResponse(this, this.getType());
	...
	public int getWorkQueue() {
		...
		if(stackObj!=null){
			try {
				...
				logger.error("NULL sessionId for RfRequest so using index:"+index);
			}
		}else{
			logger.error("NULL stack object for RfRequest so using index:"+index);
		}
		return index;
	}
	...
	//	}
	//
	//	public ShResponse createResponse() throws ShResourceException {
	//		logger.debug("createResponse() is called.");
	//		try {
	//			//if (this.getResourceContext() != null ) {
	//			if(ShResourceAdaptorImpl.getResourceContext() != null) {

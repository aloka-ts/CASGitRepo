...
public abstract class RuleObject extends Rule {
	...
	public String[] getInputData(EnumRequest message) {
		...
		for (int i = bitField.nextSetBit (0); i >= 0 && i<=(RequestHelper.varNames.length-1); i = bitField.nextSetBit (i + 1)){
			...
			if(logger.isDebugEnabled()){
				logger.debug("Request Property Name : " + RequestHelper.varNames[i]);
				logger.debug("Request Property Value : " + ruleDataArray[i]);
			}
		}

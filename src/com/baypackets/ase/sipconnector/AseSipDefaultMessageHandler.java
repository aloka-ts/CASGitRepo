/**
 * AseSipDefaultMessageHandler.java
 */
...
class AseSipDefaultMessageHandler implements AseSipMessageHandler {
	  ...
	  * remote cseq number
	  *
	  * @return positive integer if CSEQ number is valid
	  *         zero if CSEQ number is same as previous request
	  *         negative integer if CSEQ number is invalid
	  */
	 long checkCSeq(AseSipServletRequest request, AseSipSession session) {
		  return request.getDsRequest().getCSeqNumber() - session.getRemoteCSeqNumber();
	 }
	 ...
	 transient private static Logger m_logger =
		  Logger.getLogger(AseSipDefaultMessageHandler.class);
}

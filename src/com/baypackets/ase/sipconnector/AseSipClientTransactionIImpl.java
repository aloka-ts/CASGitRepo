...
/**
 * The <code>AseSipClientTransactionIImpl</code> extends
 * <code>DsSipClientTransactionIImpl</code> class and extends
 * <code>AseSipTransaction</code> interface. This class represents INVITE
 * client transactions.
 *
 * @author Neeraj Jain
 */
class AseSipClientTransactionIImpl
	extends DsSipClientTransactionIImpl
	 ...
	 *                     <code>DsSipClientTransactionIImpl</code>
	 */
	AseSipClientTransactionIImpl(DsSipRequest					request,
								DsSipClientTransactionInterface	listener,
								DsSipTransactionParams			txnParams)
		throws DsException {
		super(request, listener, txnParams);
		if(m_l.isDebugEnabled()) m_l.debug( "AseSipClientTransactionIImpl(DsSipRequest, DsSipClientTransactionInterface, DsSipTransactionParams) called");
	}
	...
	AseSipClientTransactionIImpl(DsSipRequest					request,
		...
		m_l.debug( "AseSipClientTransactionIImpl(DsSipRequest, DsSipClientTransportInfo, DsSipClientTransactionInterface) called");
	}
	...
	public void processPendingCancel(int responseCode) {
		...
		&& ((responseCode < 200) && (responseCode >= 100)) ) {
			/***
			// Check if From-tag is present in CANCEL
			String fromTag = m_cancel.getFrom().getParameter("tag");

			if(fromTag == null) {
				// Take 'tag' parameter from INVITE and set in CANCEL
				if(m_l.isDebugEnabled())
					m_l.debug("From tag not present in pending CANCEL");
				fromTag = m_request.getFrom().getParameter("tag");

				if(fromTag != null) {
					if(m_l.isDebugEnabled())
						m_l.debug("Setting From tag in pending CANCEL");
					m_cancel.getFrom().setParameter("tag", fromTag);
				} else {
					if(m_l.isEnabledFor(Level.FATAL))
						m_l.fatal("My goodness! No From tag in INVITE");
				}
			} else {
				if(m_l.isDebugEnabled())
					m_l.debug("From tag present in pending CANCEL");
			}
			***/
	...
	public synchronized void ack(DsSipAckMessage request) throws DsException, IOException, UnknownHostException {
		AseSipConstants.fixLocalIPAddress(request);
		if(m_l.isDebugEnabled()){
			if(request != null) {
				m_l.debug("bInfo.getLocalAddress(): " + request.getBindingInfo().getLocalAddress());
			}
		}
		super.ack(request);
	}
	...
	//In order to reconstruct the Client transaction APIs needs to expose
	//to set the DsSipClientTransactionInterface and DsSipClientTransportInfo
	//objects as these can't be replicated.
	/**
	 * This method associates an <code>Client Interface</code> object with this
	 * client transaction.
	 *
	 * @param client interface to be associated
	 */
	public void setClientInterface(DsSipClientTransactionInterface clientInterface) {
		if(m_l.isDebugEnabled()) m_l.debug( "setClientInterface(DsSipClientTransactionInterface) called");
		m_clientInterface = clientInterface;
	}

	/**
	 * This method returns the <code>Client Interface</code> object associated
	 * with this client transaction.
	 *
	 * @return associated client interface
	 */
	public DsSipClientTransactionInterface getClientInterface() {
		if(m_l.isDebugEnabled()) m_l.debug( "getClientInterface() called");
		return m_clientInterface;
	}
	
	/**
	 * This method associates an <code>Client Transport Info</code> object with this
	 * client transaction.
	 *
	 * @param client transport info to be associated
	 */
	public void setClientTransportInfo(DsSipClientTransportInfo clientTransportInfo) {
		if(m_l.isDebugEnabled()) m_l.debug( "setClientTransportInfo(DsSipClientTransportInfo) called");
		m_clientTransportInfo = clientTransportInfo;
	}

	/**
	 * This method returns the <code>Client Transport Info</code> object associated
	 * with this client transaction.
	 *
	 * @return associated client client transport info
	 */
	public DsSipClientTransportInfo getClientTransportInfo() {
		if(m_l.isDebugEnabled()) m_l.debug( "getClientTransportInfo() called");
		return m_clientTransportInfo;
	}
	...
	private static Logger m_l = Logger.getLogger(
								AseSipClientTransactionIImpl.class.getName());
}

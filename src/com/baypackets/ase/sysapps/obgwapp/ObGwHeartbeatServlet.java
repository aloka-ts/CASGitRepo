/*
 * ObGwHeartbeatServlet.java
 *
 * Created on July 2, 2005, 10:23 PM
 */
package com.baypackets.ase.sysapps.obgwapp;

import com.baypackets.ase.externaldevice.HeartbeatServlet;
import com.baypackets.ase.externaldevice.HeartbeatListener;
import com.baypackets.ase.externaldevice.outboundgateway.OutboundGatewayManager;
...
public class ObGwHeartbeatServlet extends SipServlet implements TimerListener, SipApplicationSessionListener, HeartbeatListener {

    private static Logger logger = Logger.getLogger(ObGwHeartbeatServlet.class);
    private static OutboundGatewayManager gwManager;
    ...
    public void init() throws ServletException {
        if (logger.isDebugEnabled()) {
            logger.debug("init(): Initializing the outbound gateway heartbeat Servlet...");
        }

        try {
        factory = (SipFactory)this.getServletContext().getAttribute(SipServlet.SIP_FACTORY);
            gwManager =
                (OutboundGatewayManager)this.getServletContext().getAttribute(OutboundGatewaySelector.class.getName());

        hbServlet = new HeartbeatServlet();
        hbServlet.init(this, this.getServletContext());

        } catch (Exception e) {
            String msg = "Error occurred while initializing the outbound gateway heartbeat Servlet: " + e.getMessage();
            logger.error(msg, e);
    ...
    public void createTimer(SipApplicationSession sess) {
        ...
        timerService.createTimer(sess, gwManager.getOutboundGatewayHeartBeatInterval() * 1000, false, null);
    }
    ...
    public URI getTo(ExternalDevice dev) throws Exception {
        InetAddress ipAddress = dev.getHost();
        return factory.createURI("sip:user@" + ipAddress.getHostAddress() + ":" + dev.getPort());
    }
    ...
    return gwManager.getRetryCount();
    }
    ...
    gwManager.outboundGatewayUp(id);
    }
    ...
    gwManager.outboundGatewayDown(id);
    }
    ...
    gwManager.outboundGatewaySuspect(id);
    }

    /**
     * Call back to determine if current role is ACTIVE
     */

...
//        return sizeOfInt;
    	...
    	//TestChanges to calculate size as  unsigned
    	int byteArrayLength =Long.toHexString(value).length();
		int sizeOfInt = ( (byteArrayLength/2) +  (byteArrayLength%2) );
		return sizeOfInt;
    ...
    public static void checkConstraints(long value, ElementInfo elementInfo) throws Exception {
    	...
    	if(value<0){
         ...
         if(logger.isDebugEnabled()){
    		...
    		logger.debug("checkConstraints inside main tcap/bn.....");
    	}
    	
        if(elementInfo.hasPreparedInfo()) {
            if(elementInfo.getPreparedInfo().hasConstraint())
                if(!elementInfo.getPreparedInfo().getConstraint().checkValue(value))
                    throw new Exception("Value of '"+elementInfo.getAnnotatedClass().toString()+"' out of bounds for value "+ value + "ElementInfo"+ elementInfo);
        }
        else {

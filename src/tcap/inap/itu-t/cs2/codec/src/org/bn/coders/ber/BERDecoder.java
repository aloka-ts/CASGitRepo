...
public class BERDecoder extends Decoder {
	...
	private static Logger logger = Logger.getLogger(BERDecoder.class); 
	...
	protected DecodedObject<Integer> decodeLength(InputStream stream)
		...
		if (bt < 128) {
			...
			//Undefined Length, these changes have been done for the
			//indefinite length for extension field for IDP (0x80)
			//The logic is to read till zeros equal to or more than
			//2 in number comes in succession and then check for the tags 
			//of other fields in IDP ForwardCallIndicators,BearerCapability,EventTypeBCSM
			//GenericNumbers
			//The commented code is for handling the indefinite length for certain parameters,
			//since the indefinite length could come for any paramater so decided to have it in
			//InapOperationCoding layer on encountering the indefinite length
			throw new IndefiniteLengthNotSupportedException("Indefinite Length");
			...
			while (stream.available() != 0) {
				...
				if (zeroCount >= 2) {
					// 9A BB 9C
					if (temp == 154 || temp == 187 || temp == 156) {
						extensionLength = extensionLength - zeroCount;
		...
		DecodedObject<Integer> obj = new DecodedObject<Integer>(result, len);
		obj.setZeroCount(zeroCount);
		// return new DecodedObject<Integer>(result,len);
		return obj;
	}
	
	
	/*protected DecodedObject<Integer> decodeOldLength(InputStream stream) throws Exception {
        int result = 0 ;        
        int bt = stream.read() ;
        if(bt == -1)
            throw new IllegalArgumentException("Unexpected EOF when decoding!");
    ...
    public DecodedObject decodeSequence(DecodedObject decodedTag,Class objectClass, 
    	...
    	if(logger.isDebugEnabled()){
    		logger.debug("decodeSequence is called for inap itu-t ");
    	}
        boolean isSet = false;
    ...
    public DecodedObject decodeSequenceOf(DecodedObject decodedTag, Class objectClass, 
        ...
        int zeroCount = len.getZeroCount();
        for (int i = 0; i < zeroCount ; i++){
        	//Ignoring zero delimiters for inknown length of extensions 128
        	stream.read();
        }
        DecodedObject<Integer> obj = new DecodedObject(result, len.getValue() + len.getSize());
        obj.setZeroCount(zeroCount);
        return obj;
    }    

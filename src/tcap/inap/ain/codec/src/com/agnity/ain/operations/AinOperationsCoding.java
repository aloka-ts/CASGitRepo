...
import com.agnity.ain.asngenerated.OriginationAttemptArg;
...
public class AinOperationsCoding {
	...
	public static Object decodeOperation(byte[] opBuffer, EventObject eventObject) throws Exception
	{
		...
		if (logger.isInfoEnabled()) {
			logger.info("decodeOperation: decoding buffer ::"+Util.formatBytes(opBuffer));
		}
		Object out = null;
	...
	public static Object decodeOperationsForOpCode(byte[] singleOpBuffer, String singleOpCode) throws Exception
	{
		...
		}else if(singleOpCode.equalsIgnoreCase(AinOpCodes.CALL_INFO_FROM_RESOURCE)){				

			if (logger.isInfoEnabled()) {
				logger.info("decodeOperationsForOpCode:decoding CALL_INFO_FROM_RESOURCE");
			}

			out = decoder.decode(ins, CallInfoFromResourceArg.class);
		}
		...
		else if(singleOpCode.equalsIgnoreCase(AinOpCodes.TERMINATION_NOTIFICATION)){
			if (logger.isInfoEnabled()) {
				logger.info("decodeOperationsForOpCode:decoding TERMINATION NOTIFICATION");
			}

			out = decoder.decode(ins, TerminationNotificationArg.class);
		//	out = decoder.decode(ins, TerminationAttemptArg.class);
		}
		else if(singleOpCode.equalsIgnoreCase(AinOpCodes.ORIGINATION_ATTEMPT)){
			if (logger.isInfoEnabled()) {
				logger.info("decodeOperationsForOpCode:decoding ORIGINATION ATTEMPT");
			}

			out = decoder.decode(ins, OriginationAttemptArg.class);
		}
        else if(singleOpCode.equalsIgnoreCase(AinOpCodes.PROVIDE_INSTRUCTION)){
			if (logger.isInfoEnabled()) {
				logger.info("decodeOperationsForOpCode:decoding PROVIDE INSTRUCTION");
			}

			out = decoder.decode(ins, ProviderInstructionArg.class);
		}
	...
	public static LinkedList<byte[]> encodeOperations(LinkedList<Object> opObjects, LinkedList<String> opCodes) throws Exception
	{
		...
		for(int i=0; i<opCodes.size(); i++)
		{
			...
			ByteArrayOutputStream outputStream = new ByteArrayOutputStream();			if(singleOpCode.equalsIgnoreCase(AinOpCodes.INFO_ANALYZED)){				if (logger.isInfoEnabled()) {					logger.info("encodeOperations:encoding INFO_ANALYZED");				}				IEncoder<InfoAnalyzedArg> encoder = CoderFactory.getInstance().newEncoder("BER");				encoder.encode((InfoAnalyzedArg)singleOpObj, outputStream);				outList.add(outputStream.toByteArray());			}			else if(singleOpCode.equalsIgnoreCase(AinOpCodes.ANALYZE_ROUTE)){				if (logger.isInfoEnabled()) {					logger.info("encodeOperations:encoding ANALYZE_ROUTE");				}				IEncoder<AnalyzeRouteArg> encoder = CoderFactory.getInstance().newEncoder("BER");				encoder.encode((AnalyzeRouteArg)singleOpObj, outputStream);				outList.add(outputStream.toByteArray());			}			else if(singleOpCode.equalsIgnoreCase(AinOpCodes.SEND_TO_RESOURCE)){				if (logger.isInfoEnabled()) {					logger.info("encodeOperations:encoding SEND_TO_RESOURCE");				}				IEncoder<SendToResourceArg> encoder = CoderFactory.getInstance().newEncoder("BER");				encoder.encode((SendToResourceArg)singleOpObj, outputStream);				outList.add(outputStream.toByteArray());			}else if(singleOpCode.equalsIgnoreCase(AinOpCodes.CALL_INFO_FROM_RESOURCE)){
				...
				if (logger.isInfoEnabled()) {
					logger.info("encodeOperations:encoding CALL_INFO_TO_RESOURCE");
				}

				IEncoder<CallInfoFromResourceArg> encoder = CoderFactory.getInstance().newEncoder("BER");
				encoder.encode((CallInfoFromResourceArg)singleOpObj, outputStream);
				outList.add(outputStream.toByteArray());
			...
			}else if(singleOpCode.equalsIgnoreCase(AinOpCodes.SEND_NOTIFICATION)){

				if (logger.isInfoEnabled()) {
					logger.info("encodeOperations:encoding Send Notification");
				}
				IEncoder<SendNotificationArg> encoder = CoderFactory.getInstance().newEncoder("BER");
				encoder.encode((SendNotificationArg)singleOpObj, outputStream);
				outList.add(outputStream.toByteArray());
			}
			else if(singleOpCode.equalsIgnoreCase(AinOpCodes.CONNECTION_CONTROL)){

				if (logger.isInfoEnabled()) {
					logger.info("encodeOperations:encoding Connection Control");
				}

			    if (singleOpObj.getClass().getSimpleName().equalsIgnoreCase("ConnectionControlArg")) {
					IEncoder<ConnectionControlArg> encoder = CoderFactory.getInstance().newEncoder("BER");
					encoder.encode((ConnectionControlArg)singleOpObj, outputStream);
					outList.add(outputStream.toByteArray());
				} else if (singleOpObj.getClass().getSimpleName().equalsIgnoreCase("TR533ConnectionControlArg")) {
					IEncoder<TR533ConnectionControlArg> encoder = CoderFactory.getInstance().newEncoder("BER");
					encoder.encode((TR533ConnectionControlArg)singleOpObj, outputStream);
					outList.add(outputStream.toByteArray());
				}	 
				
			}else if(singleOpCode.equalsIgnoreCase(AinOpCodes.ACG)){
				if (logger.isInfoEnabled()) {
					logger.info("encodeOperations:encoding ACG");
				}

				IEncoder<AcgArg> encoder = CoderFactory.getInstance().newEncoder("BER");
				encoder.encode((AcgArg)singleOpObj, outputStream);
				outList.add(outputStream.toByteArray());
			}
		}
		if (logger.isInfoEnabled()) {
	...
	/**
	 * Method for encoding Fields.
	 * @throws Exception 
	 */
	public static byte[] encodeFieldsValue(Object opObject, String opCode) throws Exception
	{
		logger.info("encodeFieldsValue:Enter");
		if(logger.isDebugEnabled()){
			logger.debug("encodeFieldsValue:Input ---> opObject:" + opObject + ", opCode:" + opCode);
		}
		byte[] opBuffer = null;
		ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
		
		//encoding FlexParameterBlockContent.
		if(opCode.equalsIgnoreCase("FlexParameterBlockContent")){
			FlexParameterBlockContent flexParamBlkContent =(FlexParameterBlockContent)opObject;
			if (logger.isInfoEnabled()) {
				logger.info("encodeFieldsValue:encoding [FlexParameterBlockContent] ");
			}
			IEncoder<FlexParameterBlockContent> encoder = CoderFactory.getInstance().newEncoder("BER");
			if (logger.isInfoEnabled()) {
				logger.info("encodeFieldsValue:encoding [FlexParameterBlockContent] get IEncoder<FlexParameterBlockContent> as::"+encoder);
			}
			encoder.encode(flexParamBlkContent, outputStream);
			if (logger.isInfoEnabled()) {
				logger.info("encodeFieldsValue:encoding [FlexParameterBlockContent]  ");
			}
			opBuffer = outputStream.toByteArray();
			if (logger.isInfoEnabled()) {
				logger.info("encodeFieldsValue:[FlexParameterBlockContent] as OutPutBuffer "+opBuffer);
			}
		}else if(opCode.equalsIgnoreCase("FlexParameterBlockContentSequenceType")){
			FlexParameterBlockContentSequenceType flexParaBlkContSeqType =(FlexParameterBlockContentSequenceType)opObject;
			if (logger.isInfoEnabled()) {
				logger.info("encodeFieldsValue:encoding FlexParameterBlockContentSequenceType");
			}
			IEncoder<FlexParameterBlockContentSequenceType> encoder = CoderFactory.getInstance().newEncoder("BER");
			encoder.encode(flexParaBlkContSeqType, outputStream);
			opBuffer = outputStream.toByteArray();
			if (logger.isInfoEnabled()) {
				logger.info("encodeFieldsValue:encoded FlexParameterBlockContentSequenceType");
			}
		}else if (opCode.equalsIgnoreCase("ApplicationErrorString")){
			if(logger.isDebugEnabled()){
				logger.debug("Inside ApplicationErrorString ecoding");
			}
			IEncoder<ApplicationErrorString> encoder = CoderFactory.getInstance().newEncoder("BER");
			encoder.encode((ApplicationErrorString)opObject, outputStream);
			opBuffer = outputStream.toByteArray();
		}

		logger.info("encodeFieldsValue:Exit:Output<-- byte[]:" + Util.formatBytes(opBuffer));
		return opBuffer;
	}
	
	/**
	 * Method use to decode field value
	 * @param ipBuffer
	 * @param fieldName
	 * @return
	 * @throws Exception
	 */
	public static Object decodeFieldsValue(byte[] ipBuffer, String fieldName) throws Exception{
		
		if(logger.isDebugEnabled()){
			logger.debug("decodeFieldsValue:Input ---> ipBuffer:" + Util.formatBytes(ipBuffer) + ", fieldName:" + fieldName);
		}
		Object out = null;
		IDecoder decoder = CoderFactory.getInstance().newDecoder("BER");
		InputStream ins = new ByteArrayInputStream(ipBuffer);	
		if(fieldName.equalsIgnoreCase("IPReturnBlockContent")){
			if (logger.isInfoEnabled()) {
				logger.info("decodeFieldsValue:decoding [IPReturnBlockContent] ");
			}
			out = decoder.decode(ins, IPReturnBlockContent.class);
			if (logger.isInfoEnabled()) {
				logger.info("decodeFieldsValue:decoded [IPReturnBlockContent] ");
			}
		}
		if(logger.isDebugEnabled()){
			logger.debug("decodeFieldsValue:exit");
		}
		return out;
	}

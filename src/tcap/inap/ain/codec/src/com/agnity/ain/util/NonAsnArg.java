...
import com.agnity.ain.enumdata.CalgPartyCatgEnum;
import com.agnity.ain.enumdata.TransmissionMedReqEnum;
...
public class NonAsnArg {
	...
	/**
	 * This function will encode tmr.
	 * @param tmrEnum
	 * @return byte[]
	 */
	public static byte[] encodeTmr(TransmissionMedReqEnum tmrEnum){
		if (logger.isInfoEnabled()) {
			logger.info("encodeTmr:Enter");
		}
		byte[] myParams = new byte[1];
		int tmr = tmrEnum.getCode();
		myParams[0] = (byte)((tmr | 0x0));
		if (logger.isDebugEnabled()) {
			logger.debug("encodeTmr:Output<--" + "\n");
			logger.debug("byte[0]-" + Util.conversiontoBinary(myParams[0]));
		}
		if (logger.isInfoEnabled()) {
			logger.info("encodeTmr:Exit");
		}
		return myParams ;
	}
	
	/**
	 * This function will decode the tmr.
	 * @param data
	 * @return TransmissionMedReqEnum object
	 * @throws InvalidInputException
	 */
	public static TransmissionMedReqEnum decodeTmr(byte[] data) throws InvalidInputException{
		if (logger.isInfoEnabled()) {
			logger.info("decodeTmr:Enter");
		}
		if(data == null){
			logger.error("decodeCalgPartyCatg: InvalidInputException(data is null)");
			throw new InvalidInputException("data is null");
		...
		int tmr = data[0]| 0x0 ;
		TransmissionMedReqEnum tmrEnum = TransmissionMedReqEnum.fromInt(tmr);
		
		if (logger.isDebugEnabled()) {
			logger.debug("decodeTmr:Output<--");
			logger.debug(tmrEnum);
		}
		if (logger.isInfoEnabled()) {
			logger.info("decodeTmr:Exit");
		}
		return tmrEnum;
	}
}

...
public class BEREncoder<T> extends Encoder<T> {    
    ...
    public int encodeBoolean(Object object, OutputStream stream, 
                                ElementInfo elementInfo) throws Exception {
        int resultSize = 1;
        stream.write( (Boolean)object ? 0x01:0x00 );
	...
	//This changes were forced as binary notes only encodes integer as signed
    //Now after couple of changes one for IDP extension field and second
    //for no answer timer we have allowed application timer integer to be encoded
    //as signed integer and all other long and integer will be encoded as unsigned ones.
    //Initially all integers and Longs were encoded as signed longs in binary notes.
    //NTT Interop: NonAnswer Timer encoding as signed integer
    //We need to encode the Application Timer value as signed then 
	//we need to call the getIntegerLength api of CoderUtil for int 
    public int encodeSignedIntegerValue(int value, OutputStream stream) throws Exception {
        int resultSize = CoderUtils.getIntegerLength(value);
        for (int i = 0 ; i < resultSize ; i++) {
            stream.write((byte)value);
            value =  value >> 8 ;
    ...
    //We need to encode the Application Timer value as signed then 
	//we need to call the getIntegerLength api of CoderUtil for int
    public int encodeSignedInteger(Object object, OutputStream stream, ElementInfo elementInfo) throws Exception{
    	int resultSize = 0;
        int szOfInt = 0;
        if(object instanceof Integer) {
            Integer value = (Integer) object;
            CoderUtils.checkConstraints(value,elementInfo);
            szOfInt = encodeSignedIntegerValue(value,stream);
        }
        else 

 ...
 *
 */
public class LAIFixedLengthCapV3 {

	private int mobCountryCode;
	private int mobNetworkCode;
	private byte[] locAreaCode;
	private static Logger logger = Logger.getLogger(LAIFixedLengthCapV3.class);
	 ...
	 */
	public static LAIFixedLengthCapV3 decode(byte[] bytes) throws InvalidInputException{
		
		if(logger.isDebugEnabled()){
			logger.debug("In decode, LAIFixedLength bytes array length::"+bytes.length);
		}
		
		LAIFixedLengthCapV3 lai = new LAIFixedLengthCapV3();
	 ...
	 * @throws InvalidInputException
	 */
	public static byte[] encode(LAIFixedLengthCapV3 lai) throws InvalidInputException{
		byte[] bytes= null;

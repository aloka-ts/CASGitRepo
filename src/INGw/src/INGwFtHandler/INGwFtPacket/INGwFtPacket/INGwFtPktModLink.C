...
INGwFtPktModLink::INGwFtPktModLink()
{
   mMsgData.msMsgType   = MSG_MOD_LINK;
	 m_isAddLinkSet       = false;
	 memset(&m_addLink, 0, sizeof(AddLink));
}

void
INGwFtPktModLink::initialize(short senderid, short receiverid, AddLink& addLnk)
{
...
int 
INGwFtPktModLink::depacketize(const char* apcData, int asSize, int version)
{
	logger.logINGwMsg(false, TRACE_FLAG, 0, "INGwFtPktModLink::depacketize");

	logger.logMsg(VERBOSE_FLAG, 0, 
	"INGwFtPktModLink::depacketize buffSize:%d, expected:%d", asSize,
...
int INGwFtPktModLink::packetize(char** apcData, int version)
{
	...
	if(!m_isAddLinkSet)
	{
		...
		"INGwFtPktModLink::packetize ModLink has not been set");
	}
	...
	logger.logMsg(VERBOSE_FLAG, 0, 
	"INGwFtPktModLink::packetize, offset:%d actualLength:%d", 
...
void
INGwFtPktModLink::getLinkData(AddLink &addLnk)
{
	memcpy(&addLnk, &m_addLink, sizeof(AddLink));
}

void
INGwFtPktModLink::setLinkData(AddLink &addLnk)
{
	m_isAddLinkSet = true;
	memcpy(&m_addLink, &addLnk, sizeof(AddLink));
}

std::string INGwFtPktModLink::toLog(void) const
{
   char buf[4000];
   int len =0;
   ...
   len += sprintf((buf + len), 
			"ModLink : LnkId:%d, LnkSetId:%d, dpcLen:%d, opc:%d, "

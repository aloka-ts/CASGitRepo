...
INGwFtPktAddLinkset::INGwFtPktAddLinkset()
{
   mMsgData.msMsgType   = MSG_ADD_LINKSET;
	 m_isAddLinksetSet    = false;
	 memset(&m_addLinkset, 0, sizeof(AddLinkSet));
}

void
INGwFtPktAddLinkset::initialize(short senderid, short receiverid,
...
int 
INGwFtPktAddLinkset::depacketize(const char* apcData, int asSize, int version)
{
	logger.logINGwMsg(false, TRACE_FLAG, 0, "INGwFtPktAddLinkset::depacketize");

	logger.logMsg(VERBOSE_FLAG, 0, 
	"INGwFtPktAddLinkset::depacketize buffSize:%d, expected:%d", asSize,
	...
	logger.logMsg(VERBOSE_FLAG, 0,
	"INGwFtPktAddLinkset::depacketize rxBufSize:%d, offset:%d", asSize, offset);

	logger.logINGwMsg(false, TRACE_FLAG, 0, "INGwFtPktAddLinkset::depacketize");
	return (offset);
}

int INGwFtPktAddLinkset::packetize(char** apcData, int version)
{
	...
	logger.logMsg(VERBOSE_FLAG, 0, 
	"INGwFtPktAddLinkset::packetize sizeAllocated:%d, size2:%d", size, size2);
	...
	if(!m_isAddLinksetSet)
	{
		...
		"INGwFtPktAddLinkset::packetize AddLinkset has not been set");
	}
	...
	logger.logMsg(VERBOSE_FLAG, 0, 
	"INGwFtPktAddLinkset::packetize, offset:%d actualLength:%d", offset, size);

	return (offset);
}

void
INGwFtPktAddLinkset::getLinksetData(AddLinkSet &addLinkset)
{
	memcpy(&addLinkset, &m_addLinkset, sizeof(AddLinkSet));
}

void
INGwFtPktAddLinkset::setLinksetData(AddLinkSet &addLinkset)
{
	m_isAddLinksetSet = true;
	memcpy(&m_addLinkset, &addLinkset, sizeof(AddLinkSet));
}

std::string INGwFtPktAddLinkset::toLog(void) const
{
   char buf[4000];
   int len =0;
   ...
   len += sprintf((buf + len), "Add Linkset Request: lnkSetId:%d,"

...
INGwFtPktAddPs::INGwFtPktAddPs()
{
   mMsgData.msMsgType   = MSG_ADD_PS;
	 m_isAddPsSet       	= false;
	 memset(&m_addPs, 0, sizeof(AddPs));
}

void
INGwFtPktAddPs::initialize(short senderid, short receiverid, AddPs& addPs)
{
...
int 
INGwFtPktAddPs::depacketize(const char* apcData, int asSize, int version)
{
	logger.logINGwMsg(false, TRACE_FLAG, 0, "INGwFtPktAddPs::depacketize");

	logger.logMsg(VERBOSE_FLAG, 0, 
	"INGwFtPktAddPs::depacketize buffSize:%d", asSize);
...
int INGwFtPktAddPs::packetize(char** apcData, int version)
{
	...
	logger.logMsg(VERBOSE_FLAG, 0, 
	"INGwFtPktAddPs::packetize sizeAllocated:%d, size2:%d", size, size2);
	...
	if(!m_isAddPsSet)
	{
		...
		"INGwFtPktAddPs::packetize AddPs has not been set");
	}
  ...
  // U32   routCtx
	...
	logger.logMsg(VERBOSE_FLAG, 0, 
	"INGwFtPktAddPs::packetize, offset:%d actualLength:%d", offset, size); 

	return (offset);
}

void
INGwFtPktAddPs::getPsData(AddPs &addPs)
{
	memcpy(&addPs, &m_addPs, sizeof(AddPs));
}

void
INGwFtPktAddPs::setPsData(AddPs &addPs)
{
	m_isAddPsSet = true;
	memcpy(&m_addPs, &addPs, sizeof(AddPs));
}

std::string INGwFtPktAddPs::toLog(void) const
{
   char buf[4000];
   int len =0;
   ...
   len += sprintf((buf + len), 
					"Add Ps: psId:%d, routCtx:%d, nwkId:%d, mode:%d, loadShareMode:%d"

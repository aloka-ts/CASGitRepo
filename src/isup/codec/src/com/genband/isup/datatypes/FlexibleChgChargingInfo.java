...
/**
 * Used for Charging Information when charging info type is 00000010
 * @author vgoel
 *
 */
public class FlexibleChgChargingInfo {

	private static Logger logger = Logger.getLogger(FlexibleChgChargingInfo.class);
	...
	/**
	 * It may be 0 (0 means activation id is not present)
	 */
	int activationId ;
	...
	/**
	 * This function will encode Flexible Charging Information. 
	 * @param chargingInfo
	 * @return byte[]
	 * @throws InvalidInputException
	 */
	public static byte[] encodeFlexibleChargingInfo(FlexibleChgChargingInfo chargingInfo) throws InvalidInputException
	{
		logger.info("encodeFlexibleChargingInfo:Enter");
		...
		if(chargingInfo.activationId != 0) {	//ext1 is 0
			byte byte1a = (byte)(chargingInfo.activationId & 0x0f);
		...
		//TODO change here for encoding bill/rate info
		...
		if(logger.isDebugEnabled())
			logger.debug("encodeFlexibleChargingInfo:Encoded Flexible Charging Info: " + Util.formatBytes(data));
		logger.info("encodeFlexibleChargingInfo:Exit");
		
		return data;
	}
	
	/**
	 * This function will decode AFlexible Charging Information.
	 * @param data
	 * @return FlexibleChgChargingInfo
	 * @throws InvalidInputException
	 */
	public static FlexibleChgChargingInfo decodeFlexibleChargingInfo(byte[] data) throws InvalidInputException
	{		
		logger.info("decodeFlexibleChargingInfo:Enter");
		if(logger.isDebugEnabled())
			logger.debug("decodeFlexibleChargingInfo: Input--> data:" + Util.formatBytes(data));
		if(data == null){
			logger.error("decodeFlexibleChargingInfo: InvalidInputException(data is null)");
			throw new InvalidInputException("data is null");
		}
		
		int index = 0;
		FlexibleChgChargingInfo chargingInfo  = new FlexibleChgChargingInfo();
		chargingInfo.setSignalElementTypeEnum(SignalElementTypeEnum.fromInt(data[index] & 0x07));
		if((data[index++]>>7 & 0x01) == 0) {		//ext1 is 0
			chargingInfo.setActivationId(data[index] & 0x0f);
		...
		//TODO change here for decoding bill/rate info

		if(logger.isDebugEnabled())
			logger.debug("decodeFlexibleChargingInfo: Output<--" + chargingInfo.toString());
		logger.info("decodeFlexibleChargingInfo:Exit");
	...
	public String toString(){
		...
		String obj = "operationClassEnum:"+ operationClassEnum + ", operationTypeEnum:"+ operationTypeEnum + ", chargedPartyTypeEnum:"+ chargedPartyTypeEnum
		 + ", chargeCollectionMethodEnum:"+ chargeCollectionMethodEnum + ", signalElementTypeEnum:"+ signalElementTypeEnum + ", activationId:"+ activationId

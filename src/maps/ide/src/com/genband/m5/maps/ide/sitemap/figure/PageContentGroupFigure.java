...
*
*     File:     PageContentGroupFigure.java
*
*     Desc:   	This a figure which is basically view for PageContentGroup.
*
...
public class PageContentGroupFigure extends BasicFigure {
	Rectangle displayPageBounds = new Rectangle(0,0,500,500);
	Color FGColor ;
	Color BGColor ;
	int yMargin ;
	public PageContentGroupFigure() {
	...
	public void paintFigure(Graphics graphics) {
		...
		Image pageContentGroupIcon = createImage("pageContentGroup16.bmp");
		...
		displayPageBounds = displayPageFigure.getBounds();
		//Rectangle siteMapBounds = getParent().getParent().getBounds();
		System.out.println("PageContentGroupFigure: paintFigure() entered");
		//System.out.println("getBounds " + getBounds());
		//System.out.println("getparent().getBounds() " + getParent().getBounds());
		
		System.out.println("PageContentGroupFigure: paintFigure() ");
		...
		xMargin = displayPageBounds.width/Constants.DISPLAYPAGE_PAGE_CONTENT_GROUP_X_MARGIN_RATIO ; //10
		yMargin = displayPageBounds.height/Constants.DISPLAYPAGE_PAGE_CONTENT_GROUP_Y_MARGIN_RATIO; //20
		
		int width = displayPageBounds.width - 2*xMargin;
		int height = displayPageBounds.height/Constants.DISPLAYPAGE_PAGE_CONTENT_GROUP_HEIGHT_RATIO;
		int x = displayPageBounds.x + xMargin;
		int y = displayPageFigure.getPageChildGroupLastY() + yMargin;
		for(int i = 0 ; i < displayPageFigure.getChildren().size() ; i++ ) { 
			if(displayPageFigure.getChildren().get(i) instanceof PageChildGroupFigure){
				hasSubPages = true;
			}
		}
		if(false == hasSubPages){
			y = displayPageBounds.y + yMargin;
		}
		//if ( Constants.PAGE_CHILD_VISIBLE == displayPageFigure.getDisplayState() ){
		
		
		
		//}else if ( Constants.PAGE_CONTENT_VISIBLE == displayPageFigure.getDisplayState() ){
		//	y = displayPageBounds.y + yMargin;
		//}
		
		bounds = new Rectangle(x,y,width,height);
		*/
		bounds = displayPageFigure.getPageContentGroupBounds();
		int x = bounds.x;
		int y = bounds.y;
		...
		*/
		...
		graphics.drawImage(pageContentGroupIcon, x + Constants.PAGE_TAB_ICON_MARGIN, y + height/2-iconHeight/2);
		graphics.setForegroundColor(ColorConstants.black);
		graphics.drawText(getPageContentGroupName(), x+2*Constants.PAGE_TAB_ICON_MARGIN+iconWidth , y + height/2-fontHeight/2);
		...
		int textXMargin = getPageContentGroupText().length()/2;
		...
		graphics.setBackgroundColor(Constants.PAGE_CONTENT_GROUP_FILL_COLOR);
		graphics.fillRectangle(fillBounds);
		//paintChildren(graphics);
		graphics.setForegroundColor(Constants.FONT_COLOR);
		graphics.drawText(getPageContentGroupText(), textX, textY );
		*/
		System.out.println("PageContentGroupFigure: paintFigure() exiting ");
	}
	/*@Override
	...
	private String getPageContentGroupName() {
		return "Page Content";
	}

...
then
  ...
  #if test -f $INSTALLROOT/BACKUP/${PATCH_NUM}/DBParamRollback.log
  #then
  #  mv $INSTALLROOT/BACKUP/${PATCH_NUM}/DBParamRollback.log $INSTALLROOT/BACKUP/${PATCH_NUM}/DBParamRollback_`date +%d%h_%H_%M_%S`.log
  #  echo "Moving file \"$INSTALLROOT/BACKUP/${PATCH_NUM}/DBParamRollback.log\" to file \"$INSTALLROOT/BACKUP/${PATCH_NUM}/DBParamRollback_`date +%d%h_%H_%M_%S`.log\""
  #fi
  #script $INSTALLROOT/BACKUP/${PATCH_NUM}/DBParamRollback.log
  #echo "PATCH NUMBER : ${PATCH_NUM}"
else
  echo "file ~/profile.ingw not found. Not logging Param Rollback Changes"
fi
echo ""
...
export MaxLWM="13000"
export LWM="12000"
export MaxInvInProgress="1500"

export OvldOnset_Lvl1="40"
export OvldAbate_Lvl1="30"
export OvldOnset_Lvl2="80"
export OvldAbate_Lvl2="70"

sqlplus -silent ${CONN_STR} << %%
  set head off;

  -- Patch NSP6.0.2
  -- Change the default CCM Normal CPU Utilization to default value of 50
  update rsicomponent_config set param_val='50', new_param_val='50' where param_oid='8.1.18' and component_id=${SubsystemId};
  --  Increased Number of CCM Worker Threads to default value of 4
  update rsicomponent_config set param_val='4', new_param_val='4' where param_oid='8.1.22' and component_id=${SubsystemId};
...
echo "Changed Max Normal CPU Utilization(8.1.18) to 50"
echo "Changed CCM Max Worker Threads(8.1.22) to 4"
#echo "Active Call MaxHWM(8.1.16), HWM(8.1.14), MaxLWM(8.1.17), LWM(8.1.15) MaxInviteInProgress(8.2.11) rollbacked successfully for SUBSYSTEM[${SubsystemId}]"

...
#Execute the build
$ANT_HOME/bin/ant sonar -Dsonar.projectKey=Rogers-CAS -Dsonar.host.url=http://10.32.10.126:9000/ -Dsonar.login=135468c9df77494b369eb4c0a26cda02c76248fd -DJAVA_HOME=${JAVA_HOME} -DINSTALLROOT=${INSTALLROOT} -DPARENT_BASE=${PARENT_BASE} -DDS_HOME=${DS_HOME} -DCAP_HOME=${CAP_HOME} -DBN_HOME=${BN_HOME} -DTHIRDPARTY=${CAS_THIRDPARTY_ROOT} -DCAS_STACK_ROOT=${CAS_STACK_ROOT} -DBAYPROCESSOR_ROOT=${BAYPROCESSOR_ROOT}  -DCOMP_MON_ROOT=${COMP_MON_ROOT} -DFEE_BUILD_ROOT=${FEE_BUILD_ROOT} -DMPH_BUILD_ROOT=${MPH_BUILD_ROOT} -DEMS_COMMON_ROOT=${EMS_COMMON_ROOT} $@

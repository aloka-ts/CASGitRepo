...
public class SOAServiceManagementView extends ViewPart 
{
		
	private TableSASSOAServiceViewer viewer =null;	
	 ...
	 * The constructor.
	 */
	public SOAServiceManagementView() 
	{
	...
	{
		viewer = new TableSASSOAServiceViewer(parent);
	...
	public void createActions()
	{
		...
		try
		{
			...
			ImageDescriptor descriptoryellow = ImageDescriptor.createFromURL(new URL(null, "file:"+new Path(SasPlugin.fullPath("icons")).append("red.gif").toOSString()));
	...
	private void showStatus()
	{
		...
		String btn[] = null;
                try
                {
                       ...
                       btn = new String[]{"OK"};
                        if(getStatusSAS.getStatus(SASAddress))
                        {
                                shutdownSAS(SASAddress);
					...
					statusASE.setAttach(0);
                                        MessageDialog msg = new MessageDialog(viewer.getControl().getShell(), "AGNITY CAS Shutdown",null, "CAS shutdown Successfully.", MessageDialog.INFORMATION, btn,0);
                                        msg.open();
                                        return;
                                ...
                                {
                                        MessageDialog msg = new MessageDialog(viewer.getControl().getShell(), "AGNITY CAS Shutdown",null, "CAS Shutdown failure.", MessageDialog.ERROR, btn,0);
                                        msg.open();
                                        return;
                                }
                        ...
                        {
                                MessageDialog msg = new MessageDialog(viewer.getControl().getShell(), "AGNITY CAS Shutdown",null, "CAS is not running.", MessageDialog.WARNING, btn,1);
                                msg.open();
                                return;
                        }
                ...
                {
			SasPlugin.getDefault().log(e.getMessage(), e);
                        MessageDialog msg = new MessageDialog(viewer.getControl().getShell(), "AGNITY CAS Shutdown",null, "CAS shutdown Failure", MessageDialog.ERROR, btn,1);
                        msg.open();
                        return;
                }
		
	}
	private void shutdownSAS(String address)
	{
...
//                      Check if the JMXMP connector is available reeta adding it
                        ...
                        ServiceManagementMBean proxy = (ServiceManagementMBean)MBeanServerInvocationHandler.newProxyInstance(mbsc, stdMBeanName, ServiceManagementMBean.class, false);
                        mbsc.invoke(stdMBeanName ,"stopserver", null, null);
                        SasPlugin.getDefault().log("I m not closing jmxc...................in showStatus");
		         	jmxc.close();
                }
                catch(Exception e)
                {
			...
			try
			{
				 SasPlugin.getDefault().log("Caught an exception so closing jmxc...................in showStatus"+e.toString());

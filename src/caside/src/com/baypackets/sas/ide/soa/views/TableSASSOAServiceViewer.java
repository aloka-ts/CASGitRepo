...
public class TableSASSOAServiceViewer {

	private Table tableOfServices = null;

	private SASDeployementSOAServicesUtil services = null;
	...
	private SOAServiceManagementView view = null;
	...
	public TableSASSOAServiceViewer(Composite parent) {
	...
	protected void showTable() {
		...
		services = SASDeployementSOAServicesUtil.getInstance();
		statusSAS = StatusASE.getInstance();
		...
		try {
			...
			ASEServices = services.getDeployedSOAServices();
			...
			// displayButtons(); //reeta added for updating the buttons when no service in table after undeploying the last servcie in table
	...
	private void deployService() {
		try {
			...
			if(IdeUtils.getSOAProjects().isEmpty()){
				String[] buttontxt = new String[] { "OK" };
				...
				MessageDialog messageBox = new MessageDialog(
						...
						"Service Deployment",
						null,
						"There is no CAS SOA Service or Application in Workspace to Deploy!!!",
...
//					}
				...
				//	jmxc.close(); commented by reeta and added this code in finally
				//		counterDeploy=0; commented by reeta and added this code in finally
				...
				messageBox.open();
				//	 if(jmxc!=null)
				jmxc.close(); //commentd by reeta and added this code in finally
	...
	public void displayButtons() {
		...
		if (this.RowIndex > -1) {
			...
			if ((tableOfServices.getItem(this.RowIndex)).getText(2).trim()
					.equals("READY")) {
				DeployButton.setEnabled(false); //added by reeta
				StartService.setEnabled(false);
				DeactivateService.setEnabled(false);
			...
			if ((tableOfServices.getItem(this.RowIndex)).getText(2).trim()
				...
				DeployButton.setEnabled(true);//added by reeta
	...
	public void setViewPart(SOAServiceManagementView view) {
		this.view = view;
	}

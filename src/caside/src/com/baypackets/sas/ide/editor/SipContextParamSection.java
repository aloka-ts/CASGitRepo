...
public class SipContextParamSection extends SectionPart implements ModelListener {
	...
	public SipContextParamSection(BPFormPage page, Composite parent,boolean sip289) {
		...
		//Create the UI.
		this.getSection().setText("Context Parameters");
		this.createControls();
	...
	protected void createControls(){
		...
		paramViewer.setContentProvider(new ContextContentProvider());
		paramViewer.setLabelProvider(new ContextLabelProvider());
	...
	public void modelChanged(int action, Node data) {
		...
		if(data != null 
				&& ( data.getNodeName().equals(CONTEXT_PARAM)
						|| (data.getParentNode() != null 
								&& data.getParentNode().getNodeName().equals(CONTEXT_PARAM) ) ) ){
			this.paramViewer.refresh();
		}
	}
	...
	public class ContextContentProvider implements IStructuredContentProvider{
		...
		public Object[] getElements(Object inputElement) {
			...
			if (!(inputElement instanceof Element))
				return new Object[0];
			NodeList list = page.getModel().getChildren( (Element)inputElement, CONTEXT_PARAM);

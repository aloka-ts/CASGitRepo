...
public class XMLEditor extends FormEditor implements ModelListener,
	...
	protected void addPages() {
		try {
			...
			SasPlugin.getDefault().log(
					"addPages() :Added source page at index " + i);

		} catch (Exception e) {
			SasPlugin.getDefault().log(
					"Exception thrown addPages() XMLEditor.java..." + e);
		}
	}
	...
	public void doSave(IProgressMonitor monitor) {
		try {
			...
			e.printStackTrace();
			SasPlugin.getDefault().log(
					"Exception thrown doSave() XMLEditor.java..." + e);
		}
	}
			...
			Display.getDefault().asyncExec(new Runnable() {
				public void run() {
					...
					for (int i = 0; i < pages.length; i++) {
						if (((FileEditorInput) editor.getEditorPart()
								.getEditorInput()).getFile().getProject()
								.equals(event.getResource())) {
							IEditorPart editorPart = pages[i].findEditor(editor
									.getEditorPart().getEditorInput());

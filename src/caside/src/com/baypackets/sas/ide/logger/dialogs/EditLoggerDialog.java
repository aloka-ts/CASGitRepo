...
import com.baypackets.sas.ide.logger.dialogs.AddSavedFilterDialog;
import  com.baypackets.sas.ide.logger.dialogs.SaveFilterDialog;

import com.baypackets.sas.ide.SasPlugin;
import com.baypackets.sas.ide.logger.filters.Filter;
import com.baypackets.sas.ide.logger.wizards.NewFilterWizard;

/**
 * The dialog that contains options for creating and editing a watcher.
 */
public class EditLoggerDialog extends Dialog
{
...
//	private Button m_wholeFileButton;
	...
	/**
	 * Constructor for EditLoggerDialog.
	 * 
	 * @param shell
	 */
	public EditLoggerDialog(Shell shell,String logstype)
	{
		super(shell);
	//	m_settings = SasPlugin.getDefault().getDialogSettings();
	...
	protected void configureShell(Shell shell)
	{
		...
		if (logstype.equals(this.SAS_LOGS)) {
			shell.setText("Edit SAS Server Logger");
		}
		else if (logstype.equals(this.DEBUG_LOGS)){
			shell.setText("Edit SIP Debug Logger");
		}
	}
...
//		if (m_editMode) {
		 //   m_fileCombo.setText(m_file.getAbsolutePath());
			...
			m_numLinesLabel.setEnabled(true);
			m_numLinesText.setEnabled(true);
			//m_wholeFileButton.setEnabled(true);
//		}
	}
...
//		m_wholeFileButton.setLayoutData(gridData);
		...
		m_numLinesLabel.setText("Number of lines (Max 100):");
		gridData = new GridData(GridData.GRAB_HORIZONTAL);
		...
		saveButton.addSelectionListener(new SelectionAdapter()
        {
            public void widgetSelected(SelectionEvent e)
            {
                ...
                if (dialog.open() == Window.OK) {
                    SasPlugin.getDefault().addSavedFilter(filter);
                }   
            }
        });
...
//			m_settings.put("numLines-" + m_file.getAbsolutePath(), m_numLines);
			//SasPlugin.getDefault().addRecentWatch(m_file.getAbsolutePath());
			super.okPressed();
		}
		else {
			// Show error msg

...
spool SipServlet5.5B04_P04.log
...
BEGIN
    ...
    BEGIN
	     ...
	     exception
            ...
            values ('SAS.Total Number of ACK Timedout', 'name','incOnly','event',0,'30.10.249','true','$SUBSYS_ID');

            insert into rsi_measurement_cntr (ID,REFTYPE,TYPE,MEASMODE,SCANINTERVAL,OID,ENABLE,SUBSYSID)
            values ('SAS.Total Number of PRACK Timedout', 'name','incOnly','event',0,'30.10.250','true','$SUBSYS_ID');

            insert into rsi_measurement_cntr (ID,REFTYPE,TYPE,MEASMODE,SCANINTERVAL,OID,ENABLE,SUBSYSID)
            values ('SAS.Total Number of Serialization Failures','name','incOnly','event',0,'30.10.251','true','$SUBSYS_ID');

            insert into rsi_measurement_cntr (ID,REFTYPE,TYPE,MEASMODE,SCANINTERVAL,OID,ENABLE,SUBSYSID)
            values ('SAS.Total Number of Deserialization Failures','name','incOnly','event',0,'30.10.252','true','$SUBSYS_ID');
																															    		  commit;
																																    END;
END;
/

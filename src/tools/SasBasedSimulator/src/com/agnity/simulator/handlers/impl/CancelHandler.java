...
public class CancelHandler extends AbstractHandler{
	...
	private static Logger logger = Logger.getLogger(CancelHandler.class);
	private static Handler handler;

	public static synchronized Handler getInstance(){
		if(handler == null){
			synchronized (CancelHandler.class) {
				if(handler ==null){
	...
	protected boolean processNode(Node node, SimCallProcessingBuffer simCpb) {
		if(logger.isInfoEnabled())
			logger.info("Inside CancelHandler processNode()");

		if(!(node.getType().equals(Constants.CANCEL))){
		...
		// Adding REL message to CANCEL request
		Node subElem =null;
		...
		while(subElemIterator.hasNext()){
			...
			if(subElem.getType().equals(Constants.BODY)){
					...
					if(bodySubElem.getType().equals(Constants.FIELD)){
						...
						if(fieldName.equals(REL_FIELD_CAUSE)){
							...
							try {
								...
								logger.error("InvalidInputException encoding Cause in REL/CANCEL request",e);
								return false;
							} 
				...
				cancelRequest = inviteReq.createCancel();
				
				
				if(leg == 1)
					simCpb.setLastSipMessageLeg1(cancelRequest);
				else if(leg == 2)
					simCpb.setLastSipMessageLeg2(cancelRequest);
				else
					simCpb.setLastSipMessage(cancelRequest);
				...
				if(relMessage != null){
					...
					try {
						...
						cancelRequest.setContent(mp,mp.getContentType());
		...
		try {
			...
			logger.error("IOException sending CANCEL request",e);
			return false;
		}

		if(logger.isInfoEnabled())
			logger.info("Leaving CancelHandler processNode() with status true");
		return true;
	}
	...
	protected boolean processRecievedMessage(Node node,
		...
		if(logger.isDebugEnabled())
			logger.debug("processRecievedMessage() for CancelHandler");

		if(!(node.getType().equals(Constants.CANCEL))){
			logger.error("Invalid Handler for node type::["+ node.getType()+"]");
			return false;
		}	
		SipServletMessage cancelReq = (SipServletMessage) message;
		
		int leg = node.getSipLeg();
		if(leg == 1)
			simCpb.setLastSipMessageLeg1(cancelReq);
		else if(leg == 2)
			simCpb.setLastSipMessageLeg2(cancelReq);
		else
			simCpb.setLastSipMessage(cancelReq);
		
		List<Node> subElements =node.getSubElements();
		if(subElements.size() == 0){
			if(logger.isDebugEnabled())
				logger.debug("CancelHandler processRecievedMessage()->No subelemnt present returning from handler");
			return true; 
		}
		
		try {			
			String contentType = cancelReq.getContentType();
			...
			if(contentType!=null && contentType.startsWith(Constants.MULTIPART_CONTENT_TYPE)){
				if(logger.isDebugEnabled())
					logger.debug("CancelHandler processRecievedMessage()->Multipart content present");
				Multipart mp = (Multipart) cancelReq.getContent();
				int bpCount =mp.getCount();
				if(logger.isDebugEnabled())
					logger.debug("CancelHandler processRecievedMessage()->Checking for ISUP content type["+Constants.ISUP_CONTENT_TYPE+"]");
				for(int i =0; i<bpCount;i++){
					BodyPart bp = mp.getBodyPart(i);
					if(logger.isDebugEnabled())
						logger.debug("CancelHandler processRecievedMessage()->Matching body part content type["+bp.getContentType()+"]");
					if(bp.getContentType().equals(Constants.ISUP_CONTENT_TYPE)){
						if(logger.isDebugEnabled())
							logger.debug("CancelHandler processRecievedMessage()->ISUP content found");
				...
				logger.error("CancelHandler processRecievedMessage()->Multipart content type not present so return with false");
				return false; 
			}
			
			if(isupContent == null){
				logger.error("CancelHandler processRecievedMessage()->ISUP content not found so return with false");
				return false; 
			}
			if(logger.isDebugEnabled())
				logger.debug("CancelHandler processRecievedMessage()->Recived REL bytes::["+Util.formatBytes(isupContent)+"]");
			...
			if(logger.isDebugEnabled())
				logger.debug("CancelHandler processRecievedMessage()->REL message parsed:: "+relMessage);
		...
		if(logger.isDebugEnabled())
			logger.debug("leaving processRecievedMessage() for CancelHandler as true");
		return true;
	}
	...
	public boolean validateMessage(Node node, Object message, SimCallProcessingBuffer simCpb) {
		...
		if(logger.isDebugEnabled())
			logger.debug("validateMessage() for CancelHandler");
		...
		if(!( node.getType().equals(Constants.CANCEL) ) ){
			if(logger.isDebugEnabled())
				logger.debug("Not a CANCEL Node");
			return false;
		}
		if(!(sipRequest.getMethod().equals("CANCEL")) ){
			if(logger.isDebugEnabled())
				logger.debug("Not a CANCEL method");
			return false;
		}
		...
		if(InapIsupSimServlet.getInstance().isB2bMode){
			...
			{
				logger.debug("Leaving validateMessage() for CancelHandler with status false as wrong leg mesaage handled");
				return false;
			}
		}
		
		if(logger.isDebugEnabled())
			logger.debug("Leaving validateMessage() for CancelHandler with status "+true);
		return true;

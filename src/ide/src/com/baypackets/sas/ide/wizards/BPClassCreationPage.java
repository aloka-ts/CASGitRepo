...
public abstract class BPClassCreationPage extends NewTypeWizardPage 
{
	...
	protected void createMethod(IType type, ImportsManager imports, 
				...
				String[] exceptions, String returnType, String contents ) 
	{
		try
		{
			...
			{
				EXCEPTIONS[i] = Signature.createTypeSignature(exceptions[i], true);
			}
			
			
			//Create comments for this method.
			String comment= !isAddComments() ? null :
					CodeGeneration.getMethodComment(type.getCompilationUnit(), type.getTypeQualifiedName('.'), methodName, paramNames, EXCEPTIONS, RET_TYPE, null, LINE_DELIMITER); 
			if (comment != null) 
			{
			...
			buffer.append(" {");
			buffer.append(LINE_DELIMITER);
			final String content= contents != null ? contents :
				CodeGeneration.getMethodBodyContent(type.getCompilationUnit(), type.getTypeQualifiedName('.'), methodName, false, "", LINE_DELIMITER);
